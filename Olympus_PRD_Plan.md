...
- **(v6.12) Mem0 적립 범위 = 1:1 DM 한정**: Mem0에 쌓이는 "대화 기억"은 **운영자와의 사적 1:1 DM 맥락**으로 한정한다. 그룹/포럼/A2A에서 나온 결정은 Mem0가 아니라 조직 지식(Obsidian, 4.3)으로 간다.
- **(v6.12) 인격은 공간 무관 항상 Mem0**: 위 적립 범위 한정은 "대화 기억"에만 적용된다. 에이전트의 **인격 자체**(성격·정체성)는 DM이든 그룹이든 언제나 동일하게 Mem0(`agent_id`)에 유지된다. 그룹의 Zeus와 DM의 Zeus는 같은 인격이다.

### 4.3 축3 — KNOWLEDGE (조직 지식) · 플랫폼 초월 공용
- **저장소**: Obsidian (Gemini가 Raw 분류/정제)
- **접근**: 에이전트는 읽기, 쓰기는 Gemini Wiki 워커 전용.
- **(v6.12) 조직 지식 적립 범위 = DM 외 전부**: 그룹/포럼/채널/A2A에서 나온 회의·협업 결정이 조직 지식이다. DM은 사적이므로 제외(4.2 참조).
- **(v6.12) 반영 경로**: 라우터 → Raw 드롭(**DM 스킵**) → Gemini Wiki 워커 → Obsidian. 라우터·에이전트는 Obsidian을 직접 쓰지 않는다(원칙 6 유지).
- **(v6.12) 반영 트리거**: Gemini 워커는 **폴링(주기 스캔)** 을 기본으로 하고, A2A `resolved`/`out` 종료 시 라우터가 Raw에 떨군 **종료 마커**를 감지하면 우선 처리한다. 반영은 **eventual**(즉시 보장 아님) — 회의 직후 DM 참조는 워커 1회전 후 보장된다.
...
### 4-A. 메모리 라이프사이클 (v6.12 — 어디에 무엇이 쌓이나)

기획 의도: **DM은 운영자를 보좌하는 사적 맥락이라 개인(Mem0), 협업·회의는 조직이 공유해야 하므로 조직(Obsidian).** 페르소나 연속성은 회의 결정을 DM에서도 알고 대화를 이어가는 구조로 성립한다.

```
[DM (1:1)]        → Mem0 (사적 보좌, agent_id 키)          [Raw 드롭 스킵]
[DM 외 (그룹/포럼/A2A)] → Raw 드롭 → Gemini 워커 → Obsidian (조직 지식)  [eventual]
[인격 자체]       → 항상 Mem0 (공간 무관, 같은 인격)
[응답 합성]       → Mem0 + Obsidian(읽기) + SPACE_MEMORY[context_key]
[페르소나 연속성] → DM의 에이전트가 Obsidian을 읽어 회의 결정을 알고 대화를 이어감
```
...
기록처는 **공간 유형**으로 분기한다(v6.12, 4-A 정합).

| 역할 | 응답 | 기록처 |
|------|:---:|------|
| `to` (DM) | 가능 | **Mem0** (사적, 최종 결론) |
| `to` (그룹/포럼/A2A) | 가능 | **Obsidian** (조직 지식, Raw→Gemini 경로, eventual) |
| `cc` | 금지 | 미기록 |

> **(v6.12 변경)** 기존 "`to`는 공간 무관 Mem0 최종 결론 기록"에서 공간별 분기로 변경. 그룹/A2A 결론은 조직 지식이므로 Obsidian으로 간다. 이에 따라 Harness T5.14("resolved→Mem0")는 "DM이면 Mem0 / 그룹이면 Obsidian"으로 갱신 대상(Harness 정합 단계, before/after 보고).
...
| 컴포넌트 | 역할 | 비고 |
|----------|------|------|
| Mem0 | PERSONA(인격/기억), `agent_id` 키 | 에이전트가 직접 연동. **(v6.12) 대화 기억 적립은 1:1 DM 한정**(인격 자체는 공간 무관) |
| 라우터 Raw 드롭 | 전 메시지 → Raw 저장소 | 옵션(yaml 토글). 백엔드: **file 기본 / sqlite 옵션**. **(v6.12) DM 스킵**(사적, 조직 파이프라인 제외) |
| Gemini Wiki Engine | Raw 분류/정제 | **(v6.12) 트리거: 폴링 기본 + A2A resolved/out 마커 우선** (eventual) |
| Obsidian | 조직 공용 지식 저장 | 플랫폼 무관. **(v6.12) 조직 지식 = DM 외 회의·협업 결정** |
...
## 9-D. 보안감사 모듈 (v6.12 — 옵션, 향후. 계약·자리만, 구현은 Phase 12)

> B2B 컴플라이언스용. LLM 대화를 불변 저장소에 적재 → 배치 감사 → 감사 보고서 제출.
> **본 절은 계약(인터페이스·정책 구조)만 정의한다. 실제 구현은 Phase 12.** 멀티테넌시(9-B)와 동일하게 "지금은 자리만, 켜기 전엔 없는 것과 동일".

### 9-D.1 싱크 분리 (지식용과 별개)
지식용 Raw Sink(9절, fire-and-forget·재시작 휘발 허용·빠름)와 감사용은 **비기능 요구가 정반대**다. 같은 싱크로 둘 다 만족할 수 없으므로 분리한다.

| 싱크 | 용도 | 특성 |
|------|------|------|
| Raw Sink (기존) | 지식 파이프라인 (Gemini→Obsidian) | fire-and-forget, 재시작 휘발 허용, DM 스킵 |
| **audit-sink (신규, 옵션)** | 컴플라이언스 감사 | **불변(immutable)·무손실·동기 쓰기 확인**. 쓰기 실패를 무시하지 않는다 |

> 두 싱크는 `raw-sink` 인터페이스 계약(`write(record)`)은 공유하되, **구현 클래스는 분리**한다(`AuditSink`는 동기 쓰기 + 무결성 보장). audit-sink는 휘발 전제를 따르지 않는다.

### 9-D.2 감사 정책 (yaml, 관리자 전용)
```yaml
audit:
  enabled: false        # 모듈 전체 토글 (기본 OFF — 켜기 전엔 없는 것과 동일)
  dm: false             # DM 감사 (기본 끔, 사적 — 켤지는 B2B 계약·법규 결정)
  org:
    enabled: true       # DM 외(조직) 감사
    default: true       # 전수 감사(1)가 기본 — opt-out 방식
    exclusions: []       # 감사 면제(0) space_id 목록 — 명시 제외만 빠짐
```

- **상태↔UI 매핑 (명문화)**: `1 = 감사 대상 = UI 체크됨(기본)` / `0 = 감사 면제 = UI 체크 해제`. 면제는 관리자만 설정한다.
- **표현**: "블랙리스트"가 아니라 **"기본 감사(default-on) + 제외 목록(opt-out)"**. 전수가 기본, 명시 제외만 빠진다.

### 9-D.3 권한 분리 (핵심 — 감사 무결성)
- 감사 정책의 설정·변경은 **`/admin/*` 관리자 전용**이다. **피감사자(일반 사용자/에이전트 대화 당사자)는 감사 정책을 보거나 바꿀 수 없다.**
- 근거: 피감사자가 자기 감사를 끌 수 있으면 감사 자체가 무의미하다(separation of duties).
- **메타 감사**: 감사 정책 변경 이력(누가·언제·어느 채널을 면제했나)도 audit-sink에 기록한다. 관리자라도 변경하면 흔적이 남는다.

### 9-D.4 판정 & 운영
- **판정**: `context_key`의 `space_type`(dm 여부) / `space_id`(채널) 매칭으로 감사 대상 여부 결정. 단순 매칭이므로 Dumb Pipe 위반 아님(파싱·LLM 아님).
- **런타임 재로드**: 감사 정책 변경은 재시작 없이 반영(registry 재로드와 동일).
- **배치 감사**: audit-sink를 입력으로 하는 별도 배치 워커(Gemini 워커와 별개 소비자)가 감사 수행 → 보고서 생성. 라우터·코어와 비동기 분리(원칙 6 유지).

### 9-D.5 미결 (B2B 계약·법규 영역)
- **DM 감사 여부**: 사적성↔감사 필요성의 충돌. 개인정보 법규·고객 계약별 결정. PRD는 토글 존재만 정의.
- **보존 기간(retention)·접근통제·보고서 포맷**: Phase 12 구현 시 결정.
...
### Phase 12 — 보안감사 모듈 (v6.12 신규, 미구현 — 옵션, B2B)
- [ ] T12.1: `audit.enabled:false` → audit-sink 비가동 (없는 것과 동일)
- [ ] T12.2: audit-sink는 동기 쓰기·무손실 (쓰기 실패 시 무시 안 함, Raw Sink와 구현 분리)
- [ ] T12.3: `audit.org.default:true` → DM 외 전수 감사, `exclusions`에 든 space_id만 면제(opt-out)
- [ ] T12.4: `audit.dm` 토글로 DM 감사 on/off
- [ ] T12.5: 감사 정책 변경은 `/admin/*` 전용 — 비관리자 접근 거부
- [ ] T12.6: 감사 정책 변경 이력이 audit-sink에 기록(메타 감사)
- [ ] T12.7: 감사 정책 런타임 재로드 (재시작 없이 반영)
- [ ] T12.8: 배치 감사 워커 → 감사 보고서 생성 (라우터 코어 비차단)

> **메모리 라이프사이클 (v6.12) 관련 테스트는 Phase 8(A2A) 정합 시 갱신**: 기존 T5.14("resolved→Mem0 기록")를 "DM이면 Mem0 / 그룹이면 Obsidian(Raw 경로)"로 변경. before/after 보고 필수(테스트 무결성 규칙: PRD 변경에 따른 정당한 수정).
...
| audit-sink | (v6.12) 보안감사용 불변·무손실 저장 싱크. raw-sink 인터페이스 공유, 구현 분리 |
...
| 메모리 라이프사이클 | (v6.12) DM=Mem0(사적) / DM외=Obsidian(조직). 인격은 공간 무관 Mem0. 회의 결정은 Raw→Gemini→Obsidian로 eventual 반영 |
...
| Gemini Wiki 트리거 시점 | **(v6.12) 결정됨** — 폴링 기본 + A2A resolved/out 마커 우선. 반영은 eventual |
| 보안감사 모듈 본격 구현(Phase 12) | 미결 — 계약은 9-D 확정. DM 감사 여부·보존기간·접근통제·보고서 포맷은 B2B 계약·개인정보 법규별 결정 |
