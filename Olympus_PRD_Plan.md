...
1. resolved   에이전트가 "결론 났음" 선언       (가장 이상적, 조기종료)
...
> 2기든 10기든 각 에이전트가 10회씩 말할 수 있으므로 참여자 수와 무관하게 라운드 보장.
> SINGLE 무한 연쇄도 동일 규칙으로 자연 방어(별도 하드캡 불필요).

**발화자 기준 비교**
```
2기 (Zeus↔Hera):   각 10회 → 최대 10라운드 ✅
3기 (Zeus↔Hera↔Athena): 각 10회 → 최대 10라운드 ✅ (기존 전역 하드캡은 3~4라운드에서 끊겼음)
N기: 각 10회 → 에이전트 수 무관 공평 ✅
```
...
### 6.5 권한 & 가드 (검증 순서)
```
1. 권한         (공통): can_initiate, allowed_targets
2. 자기호출     (공통): self-call 금지
3. 교차플랫폼   (공통): cross-platform 절대 차단
4. 조기종료     (dialogue): status == "resolved" → 즉시 정상 종료 (최우선)
5. 라운드 한도  (dialogue): round <= 10
6. 발화자 한도  (공통): speaker_counts[caller] <= 10  (최후, SINGLE/DIALOGUE 공통)
7. 스푸핑 방지  (공통): 소스 URL ↔ registry url 대조
```
...
```json
{
  "context_key": "telegram:forum:C123:42",
  "routing": { "to": ["hera"], "cc": [] },
  "memory_scope": { "space_key": "telegram:forum:C123:42", "persona_key": "hera" },
  "payload": { "origin_platform": "telegram", "text": "예산 잔액 알려줘" },
  "a2a": {
    "enabled": true,
    "mode": "single",
    "origin_agent": "zeus",
    "caller": "zeus",
    "speaker_counts": { "zeus": 1 },
...
```json
{
  "context_key": "telegram:forum:C123:42",
  "routing": { "to": ["hera"], "cc": [] },
  "payload": { "origin_platform": "telegram", "text": "서버 증설 함께 결정하자" },
  "a2a": {
    "enabled": true,
    "mode": "dialogue",
    "origin_agent": "zeus",
    "caller": "zeus",
    "speaker_counts": { "zeus": 3, "hera": 2, "athena": 2 },
    "max_speaker_calls": 10,
    "round": 2,
    "max_rounds": 10,
    "last_caller": "hera",
...
### 7.5 config/agents.yaml (단일 진실 공급원)
```yaml
system:
  a2a:
    max_speaker_calls: 10    # 발화자별 개인 한도 (SINGLE/DIALOGUE 공통)
    max_rounds: 10           # DIALOGUE 라운드 한도
...
| A2A DIALOGUE | 티키타카 (최대 10라운드, resolved 조기종료) |
| speaker_counts | 발화자별 개인 호출 카운트 (에이전트당 최대 10회) |
