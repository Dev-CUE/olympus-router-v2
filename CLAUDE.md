...
> **정합 기준: Olympus_PRD_Plan.md v6.12** (pull 통신모델 / VPS 이전 / 등록토큰 / Raw 백엔드 추상화 / 상용화 골격 SDK·테넌시·온보딩 / Google A2A 관계 / **메모리 라이프사이클(DM=Mem0·DM외=Obsidian, 4-A) / 보안감사 옵션 모듈(9-D)**)
...
- **4대 규칙:**
  ...
  3. **버전을 설계/구현으로 분리 인식.** 설계(PRD)가 앞설 수 있다(현재 설계 v6.12 / 코드 ~v6.4). 작업 시작 시 이 갭부터 본다.
  4. **테스트 무결성.** 테스트를 코드에 맞추지 않는다. 코드를 PRD에 맞춘다. 테스트 수정은 PRD 모순일 때만, 그때잠 PRD를 먼저 고친다(원칙 5).
...
라우터 코어는 텍스트를 파싱하지 않는다. 비즈니스 로직·LLM 호출·문자열 의도 분석을 일체 포함하지 않는다. 목적지(to/cc) 검증 + 일감 큐 적재 + 폴링 전달만 수행한다.
...
### 원칙 4: 작업 프로토콜
`[작업금지] 브리핑 → 수정 → 승인` 순서를 지키다. 코드 작성 전 반드시 무엇을 할지 브리핑하고 승인을 받는다. 승인 없이 코드를 생성하지 않는다.
...
### 원칙 8: 메모리 라이프사이클 (v6.12 신규)
DM은 운영자를 보좌하는 사적 맥락이라 **Mem0**(적립 1:1 한정), 그룹·포럼·A2A의 회의·협업 결정은 조직이 공유하므로 **Obsidian**이다. **인격 자체는 공간 무관 항상 Mem0**(같은 에이전트는 어디서나 같은 인격). 회의 결정의 Obsidian 반영은 라우터 Raw 드롭(**DM 스킵**) → Gemini 워커 경로로 **eventual**이며, 라우터·에이전트는 Obsidian을 직접 쓰지 않는다(원칙 6 재확인). 페르소나 연속성은 DM의 에이전트가 Obsidian을 읽어 회의 결정을 알고 대화를 잇는 방식으로 성립한다. PRD 6.4 기록 규칙도 공간별 분기(DM→Mem0 / 그룹·A2A→Obsidian).
...
> **(v6.12 메모리 라이프사이클)** PERSONA 적립(대화 기억)은 **1:1 DM 한정**(사적 보좌). 그룹·포럼·A2A 결정은 KNOWLEDGE(Obsidian, 조직)로 간다. **단 인격 자체는 공간 무관 항상 Mem0**(그룹 Zeus=DM Zeus 동일 인격). 분기 기준은 `context_key`의 `space_type==dm`. 자세한 내용은 원칙 8 참조.
...
16. (v6.12) DM 메시지를 Raw 드롭 — DM은 사적, 조직 지식 파이프라인 스킵 (`space_type==dm`)
17. (v6.12, Phase 12) 감사 정책을 피감사자가 변경 — 정책은 `/admin/*` 관리자 전용 / audit-sink에 fire-and-forget 적용 — 감사는 무손실(동기 쓰기 확인)
...
> 설계 v6.12 기준 미구현 Phase: **Phase 8**(Agora 동기화), **Phase 9**(다중 사용자·Admin API), **Phase 10**(Pull 통신 전환 — Job Queue·poll/result·등록토큰), **Phase 11**(상용화 골격 — SDK·테넌시 키·온보딩), **Phase 12**(보안감사 모듈 — audit-sink·배치 감사·보고서, B2B 옵션).
