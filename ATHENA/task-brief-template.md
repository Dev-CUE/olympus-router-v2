...
반드시 두 가지 이유를 모두 작성한다.

### 기술적 이유

`<아키텍처, 보안, 데이터 정합성, 테스트, 장애 복구, 성능 등 기술 관점>`

### 운영상 이유

`<실제 운영, QA, 유지보수, 협력사 연동, 비용/토큰 절감, Athena 이외의 LLM/코딩 에이전트 오해 방지 등 운영 관점>`
...
작업 결과가 실질 변경으로 이어졌다면 `ATHENA/change-record-template.md` 형식으로 변경 기록을 남긴다.
